...
function getTokenInfo(info: any, token: any) {
    ...
    if (info[tokenId]) {
        ...
        try {
            if (result.decimals) {
                result.balance = (result.balance / Math.pow(10, result.decimals)).toFixed(result.decimals);
            }
        } catch (error) {
            result.balance = 'N/A';
        }
...
tokenAPI.get(
    '/',
    permissionHelper(UserRole.ROOT_AUTHORITY, UserRole.USER),
    async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            if (user.role === UserRole.ROOT_AUTHORITY) {
                ...
                const info = await HederaHelper.setOperator(userID, userKey).SDK.accountInfo(user.hederaAccountId);
...
tokenAPI.put(
    '/:tokenId/associate',
    permissionHelper(UserRole.USER),
    async (req: AuthenticatedRequest, res: Response) => {
        try {
            const wallet = new Wallet();
    ...
    async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            if (!user.hederaAccountId) {
                res.status(400).json({
                    code: 400,
                    message: 'User is not linked to an Hedera Account',
                });
                return;
            }
...
tokenAPI.get(
    ...
    async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            const info = await HederaHelper.setOperator(root.hederaAccountId, root.hederaAccountKey).SDK.accountInfo(
