...
export class MessageBrokerChannel {
   ...
   */
  public async response<TData, TResponse>(
    eventType: string,
    handleFunc: (data: TData) => Promise<IMessageResponse<TResponse>>
  ) {
          ...
          async () => {
            ...
            try {
              ...
              if (m.headers.has('chunks')) {
                ...
                const requestChunksSorted = new Array<Buffer>(
   ...
   */
  public async request<T, TResponse>(
    eventType: string,
    payload: T,
    timeout?: number
  ): Promise<IMessageResponse<TResponse>> {
