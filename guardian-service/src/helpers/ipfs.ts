...
import { MessageAPI, IGetFileMessage } from "interfaces";
...
export class IPFS {
     ...
     * @param channel
     */
    public static setChannel(channel: MessageBrokerChannel): any {
    ...
    public static async addFile(file: ArrayBuffer): Promise<{ cid: string, url: string }> {
        const res = await this.channel.request<any, { cid: string, url: string }>(this.target + '.' + MessageAPI.IPFS_ADD_FILE, { content: Buffer.from(file).toString('base64') });
        if (!res) {
